...
| 16 | `ae87396` | **UI v2 Phase 0: axe baseline gains thirteen keys.** `e2e/checks/axe-modals.spec.ts` scans the states the route-level baseline never met: the My Deals and Bought Deals boards with a seeded deal and their open modals, the Analyze save modal, the liquidity transaction form and settings panel, and all six routes at 390 px. Recorded from the untouched `ui-v2-baseline` tree, so nothing is fixed here; two criticals the v1 baseline could not see are now on record (`label` ×2 on the seeded Bought board and its modal, `button-name` ×1 on My Deals at 390 px) and must read 0 after v2 Phase 3. From v2 on, `e2e/flows` and `e2e/fixtures` are golden paths too (`docs/plans/2026-09-05-ui-v2-plan.md` §2). |
| 17 | `95caa9b` | **UI v2 Phase 2: axe baseline gains eighteen keys.** `e2e/checks/theme.spec.ts` scans the dashboard and the liquidity page in every look × mode set (`landing@<look>-<mode>`, `liquidity@<look>-<mode>`, 16 keys) plus the Appearance drawer (`settings@modal`) and the command palette (`command@modal`). Recorded on chromium from the tree that mounts the shell; nothing pre-existing was re-recorded. |
| 18 | `00cc8e5` | **UI v2 Phase 3: the liquidity spec runs everywhere.** The two `test.skip(isNarrow…)` guards (recurring-series delete, Mercury breakdown) become an `openOverview()` step that expands the sidebar on phones through `liquidity.sidebar-toggle`; the `pageIsZoomedOut` guard stays but no longer fires because the toolbar wraps below `sm`. First `Golden update:` that edits a flow spec — permitted since Phase 0 moved `e2e/flows` into the golden paths. 15 passed, 0 skipped across chromium, Mobile Chrome and Mobile Safari. |
| 19 | `eb9d7fe` | **UI v2 final run archived** as `e2e/reports/v2-final.json` (five projects, 211 passed / 143 skipped / 0 failed). `compare-reports` against `phase5-final.json` (the v1 exit): 149 passed → passed, 4 skipped → passed — the narrow-viewport liquidity cases un-skipped in row 18 — 49 skipped → skipped, 152 additions (shell, theme, axe-in-every-look, geometry-free checks), 0 failures. No request golden changed anywhere in v2. |
