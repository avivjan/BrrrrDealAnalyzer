/**
 * Ambient typing for the globally registered primitives.
 *
 * `registerUiPrimitives` (see `src/components/ui/register.ts`) registers these
 * on the app, so no template imports them; without this augmentation `vue-tsc`
 * would see every `<UiButton>` as an unknown element and check nothing. The two
 * lists must stay in step — a name here that is not registered type-checks a
 * tag that fails to resolve at runtime.
 *
